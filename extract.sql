...
round((cast(extract(epoch from ne.charttime - adm.admittime)/(60*60) as numeric)), 2) as ne_adm_period ,

-- time period between note charttime and first icu visit in hours
round((cast(extract(epoch from icu.intime - ne.charttime)/(60*60) as numeric)), 2) as icu_ne_period,
...
order by pat.subject_id, adm.admittime, ne.charttime, icu.intime;
